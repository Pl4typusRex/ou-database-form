...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" onsubmit = "return validate()" name = "tt284guestform<?php echo $row["id"]; ?>" >
  ...
  <input type="text" name="notes" id="notes" placeholder="additionalnotes" maxlength="25" value="<?php if ($valid){echo htmlspecialchars($row["notes"]); }else{echo htmlspecialchars($webdata["notes"]);}?>" onchange="myFunction(this.id , this.value, 'reg')">
  <span id="fbnotes"></span><?php if (!$valid){echo  $formerror["notes"]  ;} ?>

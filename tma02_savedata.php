...
if (!empty($_POST))
{
	...
	if (isset($webdata['editid'] ) && !empty($webdata['editid'] )){
		$sql = "UPDATE $mytable SET firstname = ? , lastname= ? , email = ? , notes = ? WHERE id = ? ";
		?>
		<div class="report">Using SQL to save form: <?php echo $sql; ?></div>
		<?php
		$stmt = mysqli_prepare($dbhandle, $sql );
		mysqli_stmt_bind_param($stmt, 'sssss', $webdata['firstname'], $webdata['lastname'], $webdata['email'], $webdata['notes'], $webdata['editid']);

	}else{
	$sql = "INSERT INTO $mytable (firstname, lastname, email, notes) VALUES (?,?,?,?)";
	?>
		<div class="report">Using SQL to save form: <?php echo $sql; ?></div>
		<?php
	$stmt = mysqli_prepare($dbhandle, $sql );
	mysqli_stmt_bind_param($stmt, 'ssss', $webdata['firstname'], $webdata['lastname'], $webdata['email'], $webdata['notes']);
  }
